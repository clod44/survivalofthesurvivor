class Camera2D {
        ...
        */
        ...
        for (let y = -xCount; y <= xCount; y++) {
            for (let x = -xCount; x <= xCount; x++) {
                image(pg,
                    this.graphics.width * x + horizontalModuloOffset + xOffset,
                    this.graphics.height * y + verticalModuloOffset + yOffset,
    ...
    //////////////////////////////////////////////// Debug
    addDebugValue(label="label", valueRef="0") {
        this.debugValues.push({ label, valueRef });
    }
    ...
    updateDebugValues() {
        ...
        this.graphics.strokeWeight(1); 
        this.graphics.stroke(0);
        this.graphics.textSize(this.debugFontSize);

        let yOffset = this.debugY + this.debugFontSize;
        for (let i = 0; i < this.debugValues.length; i++) {
            const { label, valueRef } = this.debugValues[i];
            const value = typeof valueRef === 'function' ? valueRef() : valueRef;
            const displayText = `${label} : ${value}`;
            this.graphics.text(displayText, this.debugX + 5, yOffset);
            yOffset += this.debugFontSize + 2;
        }
    }
}
